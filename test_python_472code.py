...
class Gaussian:
    ...
    def cum_dist_func(self,x,y,center_x,center_y,skew):
        return (self.height/0.25)*(1-erf(skew*(center_x-x)/self.sigma))*((1-erf(skew*(center_y-y)/self.sigma)))
    ...
    def gauss(self,x,y,center_x,center_y):
        ...
        return self.height*1./(self.sigma**2*2*pi)*np.exp(-(((center_x-x)/self.sigma)**2+((center_y-y)/self.sigma)**2)/2)
...
class SimCCD:
    ...
    def __call__(self,Npoints=None,layout='baltay',width=2.,height=40000.,skew=False,skew_a=None):
        """
        ...
        if layout == 'baltay':
            coordinates = np.loadtxt('baltay.txt')
            for c in coordinates:
                result = gaussian(c[3],c[4],xygrid=self.xygrid)
                self.image += result
            coordinates = np.loadtxt('baltay2.txt')
            for c in coordinates:
